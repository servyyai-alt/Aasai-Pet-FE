    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-ocean-400 mt-1">{total} products found</p>
        </div>
        <button onClick={() => setShowFilters(!showFilters)} className="md:hidden flex items-center gap-2 btn-secondary text-sm">
          <FiFilter /> Filters
        </button>
      </div>
      ...
      <div className="sm:flex gap-6 transition-all duration-300">
        ...
        <aside className={`${showFilters ? 'block' : 'hidden'} md:block sm:w-64 flex-shrink-0 transition-all duration-500  ease-in-out`}>
          <div className="card p-5 sticky top-24">
            ...
            {/* Search */}
            <div className="mb-5">
              <label className="block text-sm font-medium text-ocean-700 mb-2">Search</label>
              <div className="relative">
                <input type="text" placeholder="Search products..." value={filters.search}
                  onChange={e => updateFilter('search', e.target.value)}
                  className="input-field text-sm pl-9 py-2.5" />
                <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-ocean-300 w-4 h-4" />
                {filters.search && <button onClick={() => updateFilter('search', '')} className="absolute right-3 top-1/2 -translate-y-1/2"><FiX className="w-4 h-4 text-ocean-300" /></button>}
              </div>
            </div>
            ...
            {/* Sort */}
            <div>
              <label className="block text-sm font-medium text-ocean-700 mb-2">Sort By</label>
              <div className="relative">
                <select value={filters.sort} onChange={e => updateFilter('sort', e.target.value)} className="input-field text-sm py-2.5 appearance-none">
                  {sortOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                <FiChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 text-ocean-400 w-4 h-4 pointer-events-none" />
              </div>
            </div>
          </div>
        </aside>
        ...
        <div className="flex-1 min-w-0">
          {loading ? (
            ...
            <div className="text-center py-20 text-ocean-400">
              ...
              <h3 className="font-display text-xl font-bold text-ocean-700 mb-2">No products found</h3>
