...
import { FiGrid, FiBox, FiShoppingBag, FiUsers, FiTag, FiBarChart2, FiLogOut, FiMenu, FiX, FiChevronRight } from 'react-icons/fi';
...
const AdminSidebar = ({ collapsed, setCollapsed }) => {
  ...
  return (
    <>
      {/* Mobile overlay */}
      {!collapsed && <div className="fixed inset-0 bg-black/50 z-20 lg:hidden" onClick={() => setCollapsed(true)} />}

      <aside className={`fixed left-0 top-0 h-full bg-ocean-900 text-white z-30 transition-all duration-300 flex flex-col
          ...
          {navItems.map(({ path, label, icon: Icon, exact }) => {
            ...
            return (
              <Link key={path} to={path}
                ...
                {!collapsed && active && <FiChevronRight className="ml-auto w-4 h-4" />}
              </Link>
            );
          })}
        </nav>
